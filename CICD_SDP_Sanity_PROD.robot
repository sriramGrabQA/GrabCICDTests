...
02. To check the working of Update Order API
    ...
    ${order_id} =       APIcalls.check_update_order     SIT
    log to console      ${order_id}
    log to console      \n reading order logs please wait one minute
    Sleep   60 seconds
    log to console      \n reading order logs please wait one minute
    @{update_order_logs}=    read_update_order_logs     SIT
...
05. To check the working of create order API in Shipsy
    log to console      \n checking the working of Create Order API in Shipsy
    @{response_create_order_shipsy} =       APIcalls.check_create_order_api_shipsy  SIT
...
06. To check the working of update order API in Shipsy
    log to console      \n checking the working of Update Order API in Shipsy
    @{response_update_order_shipsy} =       APIcalls.check_update_order_api_shipsy  SIT
